export const navLinks = [
  { href: "#features", label: "Features" },
  { href: "#progress", label: "How it Works" },
  { href: "/sign-in", label: "Login" },
];
export const features = [
    {
        icon: "🏋️‍♂️",
        title: "Exercise Library",
        description: "Easily log your workouts, sets, reps, and weights to monitor your progress over time."
    },
    {
        icon: "📊",
        title: "Progress Tracking",
        description: "Keep track of your PR and best workouts sessions."
    },
    {
        icon: "🗓️",
        title: "Workout Scheduling",
        description: "Plan your workouts in advance and see upcoming sessions at a glance."
    },
]

export const progress = [
    {
        title: "Create exercises",
        description: "Add your favorites lifts to your exercise library. Orginize them by muscle group for easy access"
    },
    {
        title: "Log your workout",
        description: "Select exercises, enter your sets, record weight and reps. Simple, fast and effective"
    },
    {
        title: "Watch growth",
        description: "View your progress and celebrate every PR. Your strength journey visualized beautifully"
    }
]

export const pageLinks = [
    {
        title: "Workouts",
        href: "/pages/workouts",
        icon: "🏠"
    },
    {
        title: "Exercises",
        href: "/pages/exercises",
        icon: "🏋️‍♂️"
    },
    {
        title: "Progress",
        href: "/pages/progress",
        icon: "📊"
    }
]
